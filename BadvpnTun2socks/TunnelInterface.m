...
+ (TunnelInterface*)sharedInterface {
    ...
    dispatch_once(&onceToken, ^{ interface = [TunnelInterface new]; });
    return interface;
}
...
- (void)writePacket:(NSData*)packet {
    dispatch_async(
        dispatch_get_main_queue(), ^{ [self->_tunnelPacketFlow writePackets:@[ packet ] withProtocols:@[ @(AF_INET) ]]; });
}
...
void _write_tunnel(uint8_t* data, int data_len, void* ctx) {
    __typeof(TunnelInterface)* selfInCb = (__bridge __typeof(TunnelInterface)*)ctx;
    [selfInCb writePacket:[NSData dataWithBytes:data length:data_len]];
}
...
- (void)_startTun2Socks:(int)socksServerPort {
    ...
    char* log_lvel = "none";
#endif
    char* argv[] = { "tun2socks", "--netif-ipaddr", "192.0.2.4", "--netif-netmask", "255.255.255.0", "--loglevel", log_lvel,
        "--socks-server-addr", socks_server };
    tun2socks_main(sizeof(argv) / sizeof(argv[0]), argv, _readFd, TunnelMTU, _write_tunnel, (__bridge void*)(self));
...
- (void)handleUdpPacket:(NSData*)packet {
    ...
    case 4: {
        ...
        ;
        dispatch_barrier_async(_readWriteQuene, ^{ self->_udpSession[key] = value; });
...
- (void)udpSocket:(GCDAsyncUdpSocket*)sock
    ...
    dispatch_sync(_readWriteQuene, ^{ strHostPort = self->_udpSession[[self strForHost:dest.addr port:dest_port]]; });

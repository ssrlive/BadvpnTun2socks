...
#define kTunnelInterfaceErrorDomain                                            \
  [NSString stringWithFormat:@"%@.TunnelInterface",                            \
...
- (instancetype)init {
  ...
  if (self) {
    ...
    _readWriteQuene =
        dispatch_queue_create("session.rw.quene", DISPATCH_QUEUE_CONCURRENT);
  ...
  if (packetFlow == nil) {
    return [NSError
        errorWithDomain:kTunnelInterfaceErrorDomain
                   code:1
               userInfo:@{
                 NSLocalizedDescriptionKey : @"PacketTunnelFlow can't be nil."
               }];
  }
...
- (void)writePacket:(NSData *)packet {
  dispatch_async(dispatch_get_main_queue(), ^{
    [self->_tunnelPacketFlow writePackets:@[ packet ]
...
- (void)processPackets {
  ...
  [_tunnelPacketFlow
      readPacketsWithCompletionHandler:^(NSArray<NSData *> *packets,
...
- (void)_startTun2Socks:(int)socksServerPort {
  ...
  char *log_lvel = "none";
#endif
  char *argv[] = {"tun2socks",       "--netif-ipaddr",      "192.0.2.4",
                  "--netif-netmask", "255.255.255.0",       "--loglevel",
                  log_lvel,          "--socks-server-addr", socks_server};
  tun2socks_main(sizeof(argv) / sizeof(argv[0]), argv, _readFd, TunnelMTU);
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kTun2SocksStoppedNotification
                    object:nil];
}
...
- (void)udpSocket:(GCDAsyncUdpSocket *)sock
  ...
  struct ip_hdr *iphdr =
      generateNewIpHeader(IP_PROTO_UDP, dest, src, total_len);
  ...
  new_udphdr->chksum =
      inet_chksum_pseudo(p_udp, IP_PROTO_UDP, p_udp->len, &odest, &osrc);
...
struct ip_hdr *generateNewIpHeader(u8_t proto, ip_addr_p_t src,
                                   ip_addr_p_t dest, uint16_t total_len) {
